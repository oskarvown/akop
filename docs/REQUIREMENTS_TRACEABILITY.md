...
| Подтверждение выбранного отдела показывает сумму долга | `tests/integration/test_bot_upload_flow.py::test_department_selection_saves_new_file` | `app/bot/handlers/upload.py` |
| Подтверждаемое undo soft-revoke (`REVOKED`), без hard-delete; повтор SHA разрешён | `tests/integration/test_bot_upload_flow.py::test_undo_requires_confirmation_then_revokes_and_allows_reassign` | `app/application/audit_service.py`, `app/bot/handlers/upload.py` |
| Undo замены атомарно восстанавливает предыдущий SUPERSEDED → ACTIVE | `tests/integration/test_audit_service_stage3.py::test_withdraw_replace_restores_previous_superseded` | `app/application/audit_service.py` |
| После 5/5 undo не предлагается; COMPLETED/EXPIRED неизменяемы | `tests/integration/test_bot_upload_flow.py::test_completed_five_of_five_does_not_offer_undo`, `test_withdraw_refuses_completed_cycle_without_reopening` | `app/bot/handlers/upload.py`, `app/application/audit_service.py` |
| Конкурентный undo vs replace: ровно один ACTIVE на отдел | `tests/integration/test_audit_service_stage3.py::test_concurrent_withdraw_and_replace_are_serialized` | `app/application/audit_service.py` |
...
| `/status` показывает все collecting и до 3 последних терминальных (COMPLETED или EXPIRED) | `tests/integration/test_bot_upload_flow.py::test_status_shows_all_collecting_cycles_and_recent_completed`, `test_idle_reminders_stage32.py::test_expired_cycle_visible_in_status_with_missing_departments` | `app/bot/handlers/status.py`, `app/application/audit_service.py` |
